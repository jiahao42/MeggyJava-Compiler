...
/**
 * We extend the DepthFirstAdapter. Visitors invoke a defaultCase method on each
 * node they visit. We override this method so that it prints out dot info about
 * a node.
 */
public class DotVisitorWithMap extends DepthFirstVisitor {
  ...
  /**
   * Upon entering each node in AST, check of this node is the root to generate
   * start of .dot file, output the dot output for the node.
   */
  ...
  /*
   * A helper output routine that generates the dot node and the dot edges from
